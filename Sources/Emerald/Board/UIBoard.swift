...
open class UIBoard: Identifiable, ObservableObject {
    ...
    public func deal(_ token: UIToken, at localPosition: CGPoint, from anchor: Anchor = .top) {
    ...
    open func nextSpace(for token: UIToken, from space: AnySpace, toward direction: Direction) -> AnySpace? {
        nil
    }
    ...
    private func handlePick(of token: UIToken, from space: AnySpace) {
    ...
    private func handleDrop(of token: UIToken, from origin: AnySpace, withOffset offset: CGSize) {
        handleDrop(of: token, from: origin, at: origin.frame.center.offset(by: offset))
    }
    
    private func handleDrop(of token: UIToken, from origin: AnySpace, at positionInBoard: CGPoint) {
        handleDrop(
            of: token,
            from: origin,
            at: positionInBoard,
    ...
    private func space(forToken token: UIToken, at localPosition: CGPoint) -> AnySpace? {
        if
