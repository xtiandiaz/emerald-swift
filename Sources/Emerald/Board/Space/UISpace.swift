...
open class UISpace<T: TokenCollection>: AnySpace {
    ...
    override func canInteract(with token: UIToken) -> Bool {
        with(token as? T.Element) { canInteract(with: $0) } without: { false }
    }
    
    override func interact(with token: UIToken) {
        with(token as? T.Element) { interact(with: $0) }
    }
    
    override func canPlace(token: UIToken) -> Bool {
        with(token as? T.Element) { canPlace(token: $0) } without: { false }
    }
    
    override func place(token: UIToken) {
    ...
    override func remove(token: UIToken) -> UIToken? {
...
open class UIStackSpace<T: UIToken>: UISpace<Stack<T>> {
...
open class UIQueueSpace<T: UIToken>: UISpace<Queue<T>> {
...
open class UIHandSpace<T: UIToken>: UISpace<Array<T>> {
